...
const { getPollData, getAllPhoneNumbersForPoll } = require(process.env
  .DB_GET_LOCATION);
...
const emailOnVote = function (poll_id) {
  ...
  getPollData(poll_id)
  .catch((err) =>
    console.log("emailOnVote had the following error : ", err)
  )
  /////// WORKING HERE 
  .then(() => {
    const mailOptions = {
    ...
    <div>Check out the current results here!<a class="link" href="Potato"></a></div>
    `,
    };
